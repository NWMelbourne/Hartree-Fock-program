...
for k,l,p,u in itertools.product(range(0,2),range(0,2),range(0,2),range(0,2)):
    for n,m,i,j in itertools.product(range(0,3),range(0,3),range(0,3),range(0,3)):
        ...
        *twointegrals(coe[m],coe[n],coe[i],coe[j],rk,rl,rp,ru+.00000000000001)
...
for n in range(0,25):
    ...
    Fp=np.matmul(np.matmul(Xt,F),X)
    eigenvalues,eigenvectors=np.linalg.eig(Fp)
    Cp= np.transpose(eigenvectors)
    C=np.matmul(X,Cp)
...
for i,j in itertools.product(range(0,2),range(0,2)):
    ...
    Erepulsone+= 0.5*P[i][j]*G[j][i]
    #E+= 0.5*(P[i][j])*(H[j][i]+F[j][i])
...
#OK, NOW ESTIMATE MINIMUM ENERGY ITERATING THROUGH VARIOUS R LENGTHS
#IF YOU ARE TRYING TO UNDERSTAND MY CODE, THIS NEXT HALF IS PRETTY MUCH
#IDENTICAL TO THE PREVIOUS HALF.
#I basically just iterate through a bunch of r values here, and append the
#energy at each step to the listts below, which are Etotal,nuclear repulsion,
#electron-nuclear attraction, and electron repulsion, and xs is the list of r
#values for plotting later.
#This could be more concise, as this half of the code is practically identical
#to the previous half, where I calculated the energy for r=1. but doesn't really matter
r=0.4
Et=[]
Enr=[]
Eea=[]
...
#This is used to make 1-d plot of the wavefunction and probability distribution
primgaussian = lambda a,rn,r : a*math.exp((-a*((rn-r)**2)))

wavefunction = lambda a,b,c,da,db,dc,ra,rb,ca,cb,r : ca*(da*primgaussian(a,ra,r) +db*primgaussian(b,ra,r) \
        +dc*primgaussian(c,ra,r)) + cb*(da*primgaussian(a,rb,r) +db*primgaussian(b,rb,r) \
        +dc*primgaussian(c,rb,r))


for w in range(0,80):
    r+=0.1
    xs.append(r*0.529177249)
    ...
    for k,l,p,u in itertools.product(range(0,2),range(0,2),range(0,2),range(0,2)):
        for n,m,i,j in itertools.product(range(0,3),range(0,3),range(0,3),range(0,3)):
            ...
            two[k][l][p][u]+= d[m]*d[n]*d[i]*d[j]\
            *twointegrals(coe[m],coe[n],coe[i],coe[j],rk,rl,rp,ru+.00000000001)
    ...
    #    P=np.array(P)
        ...
        G=np.array(G)
        F=H+G
        if n>50 and C[0][0]/C[1][0] >.99 and C[0][0]/C[1][0]< 1.01:
            status=False
        if n>800:
            status=False
            print("Unable to converge at r=" +str(r))
    Eattrac=0
    Erepuls=0
    E=0
    for i,j in itertools.product(range(0,2),range(0,2)):
        Eattrac+=P[i][j]*H[j][i]
        Erepuls+= 0.5*P[i][j]*G[j][i]

    if r> 5.499 and r<5.501:
        print(C)
    ...
    for i in range(0,100):
        x+=.2
        xaxis.append(x)
        y= wavefunction(coe[0],coe[1],coe[2],d[0],d[1],d[2],0,r,C[0][0],C[1][0],x)
        yvalue.append(y)
        probs.append(y**2)

    plt.plot(xaxis,yvalue, label= "Wavefunction",marker='None', color='black')
    plt.plot(xaxis,probs, label= "Probability Distribution",marker='None', color='red')
    plt.title("Bond length: "+str(r))
    plt.show()
    plt.close()
...
with open('nils_energy.txt', 'w') as f:
    f.write("This script computes the energy of H2 using Restricted Hartree-Fock\
 Theory with minimal basis set STO-3G.\n")
    ...
    f.write("\n")
    f.write("\n")
    f.write("The optimum bond length is found to be r="+str(xs[index])+" Angstrom.\n")
